...
class OnPolicyWithCostAlgorithm(BaseAlgorithm):
    ...
    def _setup_model(self) -> None:
        ...
        self.policy = self.policy.to(self.device)  #  ActorTwoCriticsPolicy
    ...
    def collect_rollouts(
        ...
        while n_steps < n_rollout_steps: #Perform Monte-Carlo roll-out with the policy πθ in the environment
            if self.use_sde and self.sde_sample_freq > 0 and n_steps % self.sde_sample_freq == 0:
                # Sample a new noise matrix
                self.policy.reset_noise(env.num_envs)
            ...
            if isinstance(self.action_space, gym.spaces.Discrete):
                # Reshape in case of discrete action
                actions = actions.reshape(-1, 1)
            #  Collect trajectories τb = [s0, a0, r0, gˆ0, . . . , sT , aT , rT , gˆT ]
            ...
            self._last_dones = dones
        #  Calculate  reward total returns R and reward advantages Ar
