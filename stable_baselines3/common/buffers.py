...
class RolloutBuffer(BaseBuffer):
    ...
    def add( # used in collect_rollouts
    ...
    def get(self, batch_size: Optional[int] = None) -> Generator[RolloutBufferSamples, None, None]: # used in ppolag
        assert self.full, ""
...
class RolloutBufferWithCost(BaseBuffer):
    ...
    def add(self, obs: np.ndarray, orig_obs: np.ndarray, new_obs: np.ndarray, new_orig_obs: np.ndarray,
            ...
            log_prob: th.Tensor) -> None: #used in collect_rollouts
        """
        :param obs: Observation
        :param orig_obs: Original observation
        :param new_obs: Next observation (to which agent transitions)
    ...
    def get(self, batch_size: Optional[int] = None) -> Generator[RolloutBufferSamples, None, None]:# used in ppo_lag
        assert self.full, ""
        indices = np.random.permutation(self.buffer_size * self.n_envs)
        # Prepare the data
        if not self.generator_ready:
